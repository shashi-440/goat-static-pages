...
// Support / Login instead of Blogs / Career / Visit amber.
...
/**
 * Navbar — Figma node 2456:6008.
 *
 * ── THE ANNOUNCEMENT RAIL IS GONE ──────────────────────────────────────────
 * A 42px blue strip sat above the navbar — "Download amber's UK PBSA Demand & Pricing
 * Update … Get now" — and the two travelled as one fixed stack that slid up by the rail's
 * measured height on the first scroll, so the rail left the viewport and the navbar took the
 * top edge.
 *
 * Removing it took three things with it, all of which existed only to serve it: the measured
 * `railHeight` and its resize listener, the inline `translateY` on the stack, and 42px of the
 * page's top padding (see `.page` in ListWithUs.module.scss, now 64px).
 *
 * `.stack` stays. The shared navbar module positions the <nav> itself `fixed; top: 0`, and
 * `.navbar` here hands positioning back to the stack — so the wrapper is still what pins the
 * bar, even with one child.
 */
const Header = () => {
